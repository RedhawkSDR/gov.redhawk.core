...
/**
 * This is the action bar contributor for the Prf model editor.
 */
public class PrfActionBarContributor extends EditingDomainActionBarContributor implements ISelectionChangedListener {
	/**
	...
	private final IAction showPropertiesViewAction = new Action("Show &Properties View") {
		...
		public void run() {
			try {
				...
				PrfUiPlugin.logException(exception);
			}
		}
	};
	...
	private final IAction refreshViewerAction = new Action("&Refresh") {
		...
		public boolean isEnabled() {
			if (PrfActionBarContributor.this.activeEditorPart instanceof IViewerProvider) {
				final Viewer viewer = ((IViewerProvider) PrfActionBarContributor.this.activeEditorPart).getViewer();
				if (viewer != null) {
					return true;
				}
			}
	 ...
	 */
	public PrfActionBarContributor() {
	...
	public void contributeToToolBar(final IToolBarManager toolBarManager) {
		toolBarManager.add(new Separator("prf-settings"));
		toolBarManager.add(new Separator("prf-additions"));
	}
	...
	public void contributeToMenu(final IMenuManager menuManager) {
		...
		final IMenuManager submenuManager = new MenuManager("&Prf Editor", "gov.redhawk.ui.prf.prfMenuID");
	...
	public void init(final IActionBars actionBars) {
		...
		actionBars.setGlobalActionHandler(ActionFactory.DELETE.getId(), d);
	}
