			...
			if ((selectedPosition + 1) < cis.size()) {
				...
				if ((laterComponentInstantiation.getStartOrder() != null) && (laterComponentInstantiation.getStartOrder().intValue() != 0)) {
					...
					if (editingDomain != null) {
						...
						changeOrderCmd.append(SetCommand.create(editingDomain, laterComponentInstantiation, PartitioningPackage.Literals.COMPONENT_INSTANTIATION__START_ORDER,
						        earlierStartOrder));
						changeOrderCmd.append(SetCommand.create(editingDomain, selectedCi, PartitioningPackage.Literals.COMPONENT_INSTANTIATION__START_ORDER, laterStartOrder));
