        ...
        private IStatus checkWithinRange(Range range, String value, PropertyValueType type, EStructuralFeature feature) {
            ...
            if (minObj instanceof Number && maxObj instanceof Number) {
                if (CompareNumbers.compare((Number) obj, (Number) minObj) < 0) {
                    return new EnhancedConstraintStatus((ConstraintStatus) this.ctx.createFailureStatus(value + " is less than min value of " + strMin), feature);
                } else if (CompareNumbers.compare((Number) obj, (Number) maxObj) > 0) {
                    return new EnhancedConstraintStatus((ConstraintStatus) this.ctx.createFailureStatus(value + " is greater than max value of " + strMax), feature);
                }
            }
            return null;
        }
