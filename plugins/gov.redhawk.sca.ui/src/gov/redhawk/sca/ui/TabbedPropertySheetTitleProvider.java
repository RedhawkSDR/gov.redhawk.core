...
public class TabbedPropertySheetTitleProvider extends LabelProvider {
	...
	public TabbedPropertySheetTitleProvider() {
		super();
		final IWorkbenchPart part = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().findView(ScaExplorer.VIEW_ID);
		...
		if (part != null) {
			...
			if (contentService != null) {
				...
				ScaUiPlugin.getDefault().getLog()
				        .log(new Status(IStatus.ERROR, ScaUiPlugin.PLUGIN_ID, "Could not acquire INavigatorContentService from part (\"" //$NON-NLS-1$
