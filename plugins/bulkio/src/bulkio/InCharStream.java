...
public class InCharStream extends InStreamBase {

    protected InCharPort _port;
    ...
    public ArrayDeque<InCharPort.Packet> _queue;
    
    public InCharStream(BULKIO.StreamSRI sri, InCharPort port) {
        /*
        Create an InCharStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InCharPort.Packet>();
    ...
    public CharDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public CharDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public CharDataBlock read(int count, int consume, boolean blocking) {
    ...
    public CharDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public CharDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(CharDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(CharDataBlock block, InCharPort.Packet packet)
    {
    ...
    protected CharDataBlock _readData(int count, int consume)
    {
        ...
        InCharPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        char[] empty_data = new char[0];
        CharDataBlock data = new CharDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InCharPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InCharPort.Packet packet = itr.next();
                char[] input_data = packet.dataBuffer;
    ...
    protected boolean _queuePacket(InCharPort.Packet packet) {
    ...
    protected CharDataBlock _readPacket(boolean blocking) {
        InCharPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        CharDataBlock block = new CharDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        ...
        if (this._sampleOffset != 0) {
            ...
            char[] data_payload = Arrays.copyOfRange(packet.dataBuffer, this._sampleOffset, packet.dataBuffer.length);
    ...
    protected boolean _canBridge(InCharPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InCharPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InCharPort.Packet packet = null;
    ...
    protected InCharPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InCharPort.Packet packet) {
        if (this._newstream) {
    ...
    protected void _consumeData(int count) {
        this._refreshQueue();
        while (count > 0) {
            char[] data = this._queue.peekFirst().dataBuffer;
