...
public class InULongLongStream extends InStreamBase {

    protected InULongLongPort _port;
    ...
    public ArrayDeque<InULongLongPort.Packet> _queue;
    
    public InULongLongStream(BULKIO.StreamSRI sri, InULongLongPort port) {
        /*
        Create an InULongLongStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InULongLongPort.Packet>();
    ...
    public ULongLongDataBlock read() {
        /*
    ...
    public ULongLongDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public ULongLongDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public ULongLongDataBlock read(int count, int consume, boolean blocking) {
    ...
    public ULongLongDataBlock tryread() {
        /*
    ...
    public ULongLongDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public ULongLongDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(ULongLongDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(ULongLongDataBlock block, InULongLongPort.Packet packet)
    {
    ...
    protected ULongLongDataBlock _readData(int count, int consume)
    {
        ...
        InULongLongPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        long[] empty_data = new long[0];
        ULongLongDataBlock data = new ULongLongDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InULongLongPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InULongLongPort.Packet packet = itr.next();
                long[] input_data = packet.dataBuffer;
                ...
                // remaining
                int available = input_data.length - packet_offset;
                ...
                // the next
                if (packet_offset >= input_data.length) {
                    packet_offset = 0;
    ...
    protected boolean _queuePacket(InULongLongPort.Packet packet) {
        if ((packet.EOS) && (packet.dataBuffer.length == 0)) {
            ...
            this._samplesQueued += packet.dataBuffer.length;
    ...
    protected ULongLongDataBlock _readPacket(boolean blocking) {
        InULongLongPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        ULongLongDataBlock block = new ULongLongDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        ...
        if (this._sampleOffset != 0) {
            _addTimestamp(block, _sampleOffset, 0, packet.T);
            long[] data_payload = Arrays.copyOfRange(packet.dataBuffer, this._sampleOffset, packet.dataBuffer.length);
            block.setBuffer(data_payload);
        }
        this._samplesQueued = this._samplesQueued - (packet.dataBuffer.length-this._sampleOffset);
        this._sampleOffset = 0;
    ...
    protected boolean _canBridge(InULongLongPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InULongLongPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InULongLongPort.Packet packet = null;
    ...
    protected InULongLongPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InULongLongPort.Packet packet) {
        if (this._newstream) {
    ...
    protected void _consumeData(int count) {
        this._refreshQueue();
        while (count > 0) {
            long[] data = this._queue.peekFirst().dataBuffer;
