...
import bulkio.InLongLongStream;
import bulkio.StreamListener;

/**
 * 
 */
public class InLongLongPort extends BULKIO.jni.dataLongLongPOA implements InDataPort<BULKIO.dataLongLongOperations,long[]> {
    ...
    protected Map<String, InLongLongStream> streams;
    protected Map<String, InLongLongStream[]> pendingStreams;
    protected List<StreamListener<InLongLongStream>> streamAdded = new LinkedList<StreamListener<InLongLongStream>>();
    ...
    public InLongLongPort( String portName,
                       ...
                       bulkio.sri.Comparator compareSRI,
                       bulkio.SriListener sriCallback ) {
        impl = new InPortImpl<long[]>(portName, logger, compareSRI, sriCallback, new LongLongDataHelper());
        this.streamsMutex = new Object();
        this.streams = new HashMap<String, InLongLongStream>();
        this.pendingStreams = new HashMap<String, InLongLongStream[]>();
    }
     ...
     */
    public void addStreamListener(StreamListener<InLongLongStream> listener) {
     ...
     */
    public void removeStreamListener(StreamListener<InLongLongStream> listener) {
    ...
    public InLongLongStream getStream(String streamID)
    {
        InLongLongStream stream = null;
    ...
    public InLongLongStream[] getStreams()
    {
        InLongLongStream[] retval = null;
        Iterator<InLongLongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            retval = new InLongLongStream[streams.size()];
            int streams_idx = 0;
    ...
    {
        InLongLongStream stream = new InLongLongStream(sri, this);
        synchronized (this.streamsMutex) {
            if (!streams.containsKey(streamID)) {
                // New stream
                ...
                if (!pendingStreams.containsKey(streamID)) {
                    pendingStreams.put(streamID, new InLongLongStream[0]);
                }
                InLongLongStream[] tmp_streams = Arrays.copyOf(pendingStreams.get(streamID), pendingStreams.get(streamID).length+1);
                tmp_streams[tmp_streams.length - 1] = stream;
                pendingStreams.replace(streamID, tmp_streams);
            }
            for (StreamListener<InLongLongStream> listener : streamAdded) {
                listener.newStream(stream);
            }
        }
    }

    public InLongLongStream getCurrentStream(float timeout)
    {
      // Prefer a stream that already has buffered data
      InLongLongStream retval = null;
      synchronized (this.streamsMutex) {
        for (InLongLongStream value : streams.values()) {
    ...
    public boolean isStreamEnabled(String streamID)
    {
        synchronized (this.streamsMutex) {
            if (!pendingStreams.containsKey(streamID)) {
                InLongLongStream stream = streams.get(streamID);
                if (stream != null) {
                    if (!stream.enabled()) {
                        return false;
                    }
    ...
    public InLongLongStream[] getReadyStreams(int samples)
    {
        InLongLongStream[] retval = null;
        List<InLongLongStream> stream_list = new ArrayList<InLongLongStream>();
        Iterator<InLongLongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            while (streams_iter.hasNext()) {
                InLongLongStream _stream = streams_iter.next();
                if (samples == 0) {
                    if (_stream.ready()) {
            ...
            int streams_idx = 0;
            retval = new InLongLongStream[stream_list.size()];
    ...
    protected boolean _acceptPacket(String streamID, boolean EOS)
    {
        ...
        synchronized (this.streamsMutex) {
            ...
            if (EOS) {
                ...
                InLongLongStream[] _pS = pendingStreams.get(streamID);
                if (_pS != null) {
                    streams.put(streamID, _pS[0]);
                    InLongLongStream[] tmp_streams = Arrays.copyOfRange(pendingStreams.get(streamID), 1, pendingStreams.get(streamID).length);
                    pendingStreams.replace(streamID, tmp_streams);
    ...
    protected void _removeStream(String streamID)
    {
        synchronized (this.streamsMutex) {
            ...
            InLongLongStream value = streams.get(streamID);
            if (value != null) {
                value._close();
                streams.remove(streamID);
            }
