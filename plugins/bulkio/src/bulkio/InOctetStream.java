...
public class InOctetStream extends InStreamBase {

    protected InOctetPort _port;
    ...
    public ArrayDeque<InOctetPort.Packet> _queue;
    
    public InOctetStream(BULKIO.StreamSRI sri, InOctetPort port) {
        /*
        Create an InOctetStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InOctetPort.Packet>();
    ...
    public OctetDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public OctetDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public OctetDataBlock read(int count, int consume, boolean blocking) {
    ...
    public OctetDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public OctetDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(OctetDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(OctetDataBlock block, InOctetPort.Packet packet)
    {
    ...
    protected OctetDataBlock _readData(int count, int consume)
    {
        ...
        InOctetPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        byte[] empty_data = new byte[0];
        OctetDataBlock data = new OctetDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InOctetPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InOctetPort.Packet packet = itr.next();
                byte[] input_data = packet.dataBuffer;
    ...
    protected boolean _queuePacket(InOctetPort.Packet packet) {
    ...
    protected OctetDataBlock _readPacket(boolean blocking) {
        InOctetPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        OctetDataBlock block = new OctetDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        ...
        if (this._sampleOffset != 0) {
            ...
            byte[] data_payload = Arrays.copyOfRange(packet.dataBuffer, this._sampleOffset, packet.dataBuffer.length);
    ...
    protected boolean _canBridge(InOctetPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InOctetPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InOctetPort.Packet packet = null;
    ...
    protected InOctetPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InOctetPort.Packet packet) {
        if (this._newstream) {
    ...
    protected void _consumeData(int count) {
        this._refreshQueue();
        while (count > 0) {
            byte[] data = this._queue.peekFirst().dataBuffer;
