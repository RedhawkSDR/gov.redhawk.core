 ...
 * 
 */
public class OutUInt16Port extends BULKIO.UsesPortStatisticsProviderPOA {
     ...
     * @generated
     */
    protected Map<String, dataUshortOperations> outConnections = null;
     ...
     */
    protected static int MAX_SAMPLES_PER_PUSH = MAX_PAYLOAD_SIZE/UInt16Size.bytes();
    ...
    public OutUInt16Port(String portName,
        ...
        active = false;
        outConnections = new HashMap<String, dataUshortOperations>();
     ...
     * @generated
     */
    public HashMap<String, dataUshortOperations> getPorts() {
        return new HashMap<String, dataUshortOperations>();
    }
	...
	if ( logger != null ) {
		...
		// for each connection
		for (Entry<String, dataUshortOperations> p : this.outConnections.entrySet()) {
            ...
            String streamID)
    {
        ...
        if (sriStruct.sri.subsize != 0) {
            if (MAX_SAMPLES_PER_PUSH%sriStruct.sri.subsize != 0) {
                MAX_SAMPLES_PER_PUSH = MAX_PAYLOAD_SIZE/UInt16Size.bytes();
        ...
        for (int offset = 0; offset < data.length;) {
            ...
            short[] subPacket = Arrays.copyOfRange(data, offset, offset+pushSize);
            offset += pushSize;
     ...
     * @generated
     */
    public void pushPacket(short[] data, PrecisionUTCTime time, boolean endOfStream, String streamID)
    {
        if ( logger != null ) {
    ...
    public void connectPort(final org.omg.CORBA.Object connection, final String connectionId) throws CF.PortPackage.InvalidPort, CF.PortPackage.OccupiedPort
    {
        ...
        synchronized (this.updatingPortsLock) {
            final dataUshortOperations port;
            try {
                port = BULKIO.jni.dataUshortHelper.narrow(connection);
            ...
            this.active = true;
            this.stats.put(connectionId, new linkStatistics( this.name, new UInt16Size() ) );
    ...
    public void disconnectPort(String connectionId) {
        ...
        synchronized (this.updatingPortsLock) {
            ...
            dataUshortOperations port = this.outConnections.remove(connectionId);
            if (port != null)
            {
                short[] odata = new short[0];
    ...
    public UsesConnection[] connections() {
        ...
        synchronized (this.updatingPortsLock) {
            for (Entry<String, dataUshortOperations> ent : this.outConnections.entrySet()) {
