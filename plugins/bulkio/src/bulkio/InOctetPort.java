...
import bulkio.InOctetStream;
import bulkio.StreamListener;

/**
 * 
 */
public class InOctetPort extends BULKIO.jni.dataOctetPOA implements InDataPort<BULKIO.dataOctetOperations,byte[]> {

    /**
     * A class to hold packet data.
     * 
     */
    public class Packet extends DataTransfer < byte[] > {

        public Packet(byte[] data, PrecisionUTCTime time, boolean endOfStream, String streamID, StreamSRI H, boolean sriChanged, boolean inputQueueFlushed ) {
    ...
    private InPortImpl<byte[]> impl;
    protected Map<String, InOctetStream> streams;
    protected Map<String, InOctetStream[]> pendingStreams;
    protected List<StreamListener<InOctetStream>> streamAdded = new LinkedList<StreamListener<InOctetStream>>();
    ...
    public InOctetPort( String portName,
                       ...
                       bulkio.sri.Comparator compareSRI,
                       bulkio.SriListener sriCallback ) {
        impl = new InPortImpl<byte[]>(portName, logger, compareSRI, sriCallback, new OctetDataHelper());
        this.streamsMutex = new Object();
        this.streams = new HashMap<String, InOctetStream>();
        this.pendingStreams = new HashMap<String, InOctetStream[]>();
    }
     ...
     */
    public void addStreamListener(StreamListener<InOctetStream> listener) {
     ...
     */
    public void removeStreamListener(StreamListener<InOctetStream> listener) {
    ...
    public void pushSRI(StreamSRI header) {
        synchronized (impl.sriUpdateLock) {
            if (!impl.currentHs.containsKey(header.streamID)) {
                ...
                int eos_count = 0;
                synchronized (impl.dataBufferLock) {
                    for (DataTransfer<byte[]> packet : impl.workQueue) {
    ...
    {
        DataTransfer<byte[]> p = impl.getPacket(wait);
        if (p == null) {
            return null;
        } else {
    ...
    public InOctetStream getStream(String streamID)
    {
        InOctetStream stream = null;
    ...
    public InOctetStream[] getStreams()
    {
        InOctetStream[] retval = null;
        Iterator<InOctetStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            retval = new InOctetStream[streams.size()];
            int streams_idx = 0;
    ...
    {
        InOctetStream stream = new InOctetStream(sri, this);
        synchronized (this.streamsMutex) {
            if (!streams.containsKey(streamID)) {
                // New stream
                ...
                if (!pendingStreams.containsKey(streamID)) {
                    pendingStreams.put(streamID, new InOctetStream[0]);
                }
                InOctetStream[] tmp_streams = Arrays.copyOf(pendingStreams.get(streamID), pendingStreams.get(streamID).length+1);
                tmp_streams[tmp_streams.length - 1] = stream;
                pendingStreams.replace(streamID, tmp_streams);
            }
            for (StreamListener<InOctetStream> listener : streamAdded) {
                listener.newStream(stream);
            }
        }
    }

    public InOctetStream getCurrentStream(float timeout)
    {
      // Prefer a stream that already has buffered data
      InOctetStream retval = null;
      synchronized (this.streamsMutex) {
        for (InOctetStream value : streams.values()) {
    ...
    public Packet peekPacket(float timeout)
    {
        ...
        DataTransfer<byte[]> p = impl.peekPacket(timeout_ms);
        if (p != null) {
            return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
        }
        return null;
    ...
    {
        DataTransfer<byte[]> p =  this.impl.fetchPacket(streamID);
        if (p == null) {
            return null;
        }
        return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
    ...
    public boolean isStreamEnabled(String streamID)
    {
        synchronized (this.streamsMutex) {
            if (!pendingStreams.containsKey(streamID)) {
                InOctetStream stream = streams.get(streamID);
                if (stream != null) {
                    if (!stream.enabled()) {
                        return false;
                    }
    ...
    public InOctetStream[] getReadyStreams(int samples)
    {
        InOctetStream[] retval = null;
        List<InOctetStream> stream_list = new ArrayList<InOctetStream>();
        Iterator<InOctetStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            while (streams_iter.hasNext()) {
                InOctetStream _stream = streams_iter.next();
                if (samples == 0) {
                    if (_stream.ready()) {
            ...
            int streams_idx = 0;
            retval = new InOctetStream[stream_list.size()];
    ...
    protected boolean _acceptPacket(String streamID, boolean EOS)
    {
        ...
        synchronized (this.streamsMutex) {
            ...
            if (EOS) {
                ...
                InOctetStream[] _pS = pendingStreams.get(streamID);
                if (_pS != null) {
                    streams.put(streamID, _pS[0]);
                    InOctetStream[] tmp_streams = Arrays.copyOfRange(pendingStreams.get(streamID), 1, pendingStreams.get(streamID).length);
                    pendingStreams.replace(streamID, tmp_streams);
    ...
    protected void _removeStream(String streamID)
    {
        synchronized (this.streamsMutex) {
            ...
            InOctetStream value = streams.get(streamID);
            if (value != null) {
                value._close();
                streams.remove(streamID);
            }
