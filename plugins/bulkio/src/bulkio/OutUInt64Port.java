 ...
 * 
 */
public class OutUInt64Port extends BULKIO.UsesPortStatisticsProviderPOA {
     ...
     * @generated
     */
    protected Map<String, dataUlongLongOperations> outConnections = null;
     ...
     */
    protected static int MAX_SAMPLES_PER_PUSH = MAX_PAYLOAD_SIZE/UInt64Size.bytes();
    ...
    public OutUInt64Port(String portName,
        ...
        active = false;
        outConnections = new HashMap<String, dataUlongLongOperations>();
     ...
     * @generated
     */
    public HashMap<String, dataUlongLongOperations> getPorts() {
        return new HashMap<String, dataUlongLongOperations>();
    }
	...
	if ( logger != null ) {
		...
		// for each connection
                for (Entry<String, dataUlongLongOperations> p : this.outConnections.entrySet()) {
            ...
            String streamID)
    {
        ...
        if (sriStruct.sri.subsize != 0) {
            if (MAX_SAMPLES_PER_PUSH%sriStruct.sri.subsize != 0) {
                MAX_SAMPLES_PER_PUSH = MAX_PAYLOAD_SIZE/UInt64Size.bytes();
        ...
        for (int offset = 0; offset < data.length;) {
            ...
            long[] subPacket = Arrays.copyOfRange(data, offset, offset+pushSize);
            offset += pushSize;
    ...
    public void connectPort(final org.omg.CORBA.Object connection, final String connectionId) throws CF.PortPackage.InvalidPort, CF.PortPackage.OccupiedPort
    {
        ...
        synchronized (this.updatingPortsLock) {
            final dataUlongLongOperations port;
            try {
                port = BULKIO.jni.dataUlongLongHelper.narrow(connection);
            ...
            this.active = true;
            this.stats.put(connectionId, new linkStatistics( this.name, new UInt64Size() ) );
    ...
    public void disconnectPort(String connectionId) {
        ...
        synchronized (this.updatingPortsLock) {
            ...
            dataUlongLongOperations port = this.outConnections.remove(connectionId);
            if (port != null)
            {
    ...
    public UsesConnection[] connections() {
        ...
        synchronized (this.updatingPortsLock) {
            for (Entry<String, dataUlongLongOperations> ent : this.outConnections.entrySet()) {
