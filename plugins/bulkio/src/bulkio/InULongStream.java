...
public class InULongStream extends InStreamBase {

    protected InULongPort _port;
    ...
    public ArrayDeque<InULongPort.Packet> _queue;
    
    public InULongStream(BULKIO.StreamSRI sri, InULongPort port) {
        /*
        Create an InULongStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InULongPort.Packet>();
    ...
    public ULongDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public ULongDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public ULongDataBlock read(int count, int consume, boolean blocking) {
    ...
    public ULongDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public ULongDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(ULongDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(ULongDataBlock block, InULongPort.Packet packet)
    {
    ...
    protected ULongDataBlock _readData(int count, int consume)
    {
        ...
        InULongPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        int[] empty_data = new int[0];
        ULongDataBlock data = new ULongDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InULongPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InULongPort.Packet packet = itr.next();
                int[] input_data = packet.dataBuffer;
    ...
    protected boolean _queuePacket(InULongPort.Packet packet) {
    ...
    protected ULongDataBlock _readPacket(boolean blocking) {
        InULongPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        ULongDataBlock block = new ULongDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        ...
        if (this._sampleOffset != 0) {
            ...
            int[] data_payload = Arrays.copyOfRange(packet.dataBuffer, this._sampleOffset, packet.dataBuffer.length);
    ...
    protected boolean _canBridge(InULongPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InULongPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InULongPort.Packet packet = null;
    ...
    protected InULongPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InULongPort.Packet packet) {
        if (this._newstream) {
    ...
    protected void _consumeData(int count) {
        this._refreshQueue();
        while (count > 0) {
            int[] data = this._queue.peekFirst().dataBuffer;
