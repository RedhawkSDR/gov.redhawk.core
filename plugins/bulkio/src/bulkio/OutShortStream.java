...
import bulkio.OutShortPort;

import java.util.Arrays;

public class OutShortStream extends OutStreamBase {

    protected OutShortPort _port;

    public OutShortStream(BULKIO.StreamSRI sri, OutShortPort port) {
        super(sri);
        this._port = port;
        _bufferSize = 0;
        _bufferOffset = 0;
    ...
    protected void _writeMultiple(CF.complexShort[] cx_data, bulkio.SampleTimestamp[] times, boolean eos) {
        if (this.checkSRI()) {
            this._port.pushSRI(this._sri);
        }
    ...
    protected void _doBuffer(CF.complexShort[] cx_data, BULKIO.PrecisionUTCTime time) {
        short[] data = new short[cx_data.length*2];
        for (int cx_data_idx = 0; cx_data_idx < cx_data.length; cx_data_idx++) {
            data[cx_data_idx*2] = cx_data[cx_data_idx].real;
            data[cx_data_idx*2+1] = cx_data[cx_data_idx].imag;
     ...
     * If there are any pending SRI changes, the new SRI is pushed first.
     */
    public void write(CF.complexShort[] data, BULKIO.PrecisionUTCTime time) {
        if ((this._bufferSize == 0) || ((this._bufferWritePtr == 0) && (data.length >= this._bufferSize))) {
        //if (this._bufferSize == 0) {
                bulkio.SampleTimestamp[] times = new SampleTimestamp[1];
            times[0] = new bulkio.SampleTimestamp(time, 0, false);
     ...
     * If there are any pending SRI changes, the new SRI is pushed first.
     */
    public void write(CF.complexShort[] data, bulkio.SampleTimestamp[] times) {
        if ((this._bufferSize == 0) || ((this._bufferWritePtr == 0) && (data.length >= this._bufferSize))) {
        //if (this._bufferSize == 0) {
                this._writeMultiple(data, times, false);
            return;
        ...
        for (int times_idx = 0; times_idx < times.length; times_idx++) {
            if (times.length == 1) {
                ...
                CF.complexShort[] _data = Arrays.copyOfRange(data, times[times_idx].offset, end_offset);
                this._doBuffer(_data, times[times_idx].time);
            }
        }
    };
