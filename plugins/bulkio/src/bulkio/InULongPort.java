...
import bulkio.InULongStream;
import bulkio.StreamListener;

/**
 * 
 */
public class InULongPort extends BULKIO.jni.dataUlongPOA implements InDataPort<BULKIO.dataUlongOperations,int[]> {
    ...
    private InPortImpl<int[]> impl;
    protected Map<String, InULongStream> streams;
    protected Map<String, InULongStream[]> pendingStreams;
    protected List<StreamListener<InULongStream>> streamAdded = new LinkedList<StreamListener<InULongStream>>();
    ...
    public InULongPort( String portName,
                       ...
                       bulkio.sri.Comparator compareSRI,
                       bulkio.SriListener sriCallback ) {
        impl = new InPortImpl<int[]>(portName, logger, compareSRI, sriCallback, new ULongDataHelper());
        this.streamsMutex = new Object();
        this.streams = new HashMap<String, InULongStream>();
        this.pendingStreams = new HashMap<String, InULongStream[]>();
    }
     ...
     */
    public void addStreamListener(StreamListener<InULongStream> listener) {
     ...
     */
    public void removeStreamListener(StreamListener<InULongStream> listener) {
    ...
    public void pushSRI(StreamSRI header) {
        synchronized (impl.sriUpdateLock) {
            if (!impl.currentHs.containsKey(header.streamID)) {
                ...
                int eos_count = 0;
                synchronized (impl.dataBufferLock) {
                    for (DataTransfer<int[]> packet : impl.workQueue) {
    ...
    {
        DataTransfer<int[]> p = impl.getPacket(wait);
        if (p == null) {
            return null;
        } else {
    ...
    public InULongStream getStream(String streamID)
    {
        InULongStream stream = null;
    ...
    public InULongStream[] getStreams()
    {
        InULongStream[] retval = null;
        Iterator<InULongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            retval = new InULongStream[streams.size()];
            int streams_idx = 0;
    ...
    {
        InULongStream stream = new InULongStream(sri, this);
        synchronized (this.streamsMutex) {
            if (!streams.containsKey(streamID)) {
                // New stream
                ...
                if (!pendingStreams.containsKey(streamID)) {
                    pendingStreams.put(streamID, new InULongStream[0]);
                }
                InULongStream[] tmp_streams = Arrays.copyOf(pendingStreams.get(streamID), pendingStreams.get(streamID).length+1);
                tmp_streams[tmp_streams.length - 1] = stream;
                pendingStreams.replace(streamID, tmp_streams);
            }
            for (StreamListener<InULongStream> listener : streamAdded) {
                listener.newStream(stream);
            }
        }
    }

    public InULongStream getCurrentStream(float timeout)
    {
      // Prefer a stream that already has buffered data
      InULongStream retval = null;
      synchronized (this.streamsMutex) {
        for (InULongStream value : streams.values()) {
    ...
    public Packet peekPacket(float timeout)
    {
        ...
        DataTransfer<int[]> p = impl.peekPacket(timeout_ms);
        if (p != null) {
            return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
        }
        return null;
    ...
    {
        DataTransfer<int[]> p =  this.impl.fetchPacket(streamID);
        if (p == null) {
            return null;
        }
        return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
    ...
    public boolean isStreamEnabled(String streamID)
    {
        synchronized (this.streamsMutex) {
            if (!pendingStreams.containsKey(streamID)) {
                InULongStream stream = streams.get(streamID);
                if (stream != null) {
                    if (!stream.enabled()) {
                        return false;
                    }
    ...
    public InULongStream[] getReadyStreams(int samples)
    {
        InULongStream[] retval = null;
        List<InULongStream> stream_list = new ArrayList<InULongStream>();
        Iterator<InULongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            while (streams_iter.hasNext()) {
                InULongStream _stream = streams_iter.next();
                if (samples == 0) {
                    if (_stream.ready()) {
            ...
            int streams_idx = 0;
            retval = new InULongStream[stream_list.size()];
    ...
    protected boolean _acceptPacket(String streamID, boolean EOS)
    {
        ...
        synchronized (this.streamsMutex) {
            ...
            if (EOS) {
                ...
                InULongStream[] _pS = pendingStreams.get(streamID);
                if (_pS != null) {
                    streams.put(streamID, _pS[0]);
                    InULongStream[] tmp_streams = Arrays.copyOfRange(pendingStreams.get(streamID), 1, pendingStreams.get(streamID).length);
                    pendingStreams.replace(streamID, tmp_streams);
    ...
    protected void _removeStream(String streamID)
    {
        synchronized (this.streamsMutex) {
            ...
            InULongStream value = streams.get(streamID);
            if (value != null) {
                value._close();
                streams.remove(streamID);
            }
