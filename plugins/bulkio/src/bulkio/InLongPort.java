...
import bulkio.InLongStream;
import bulkio.StreamListener;

/**
 * 
 */
public class InLongPort extends BULKIO.jni.dataLongPOA implements InDataPort<BULKIO.dataLongOperations,int[]> {
    ...
    protected Map<String, InLongStream> streams;
    protected Map<String, InLongStream[]> pendingStreams;
    protected List<StreamListener<InLongStream>> streamAdded = new LinkedList<StreamListener<InLongStream>>();
    ...
    public InLongPort( String portName, Logger logger ) {
    ...
    public InLongPort( String portName,
                       ...
                       bulkio.sri.Comparator compareSRI,
                       bulkio.SriListener sriCallback ) {
        impl = new InPortImpl<int[]>(portName, logger, compareSRI, sriCallback, new LongDataHelper());
        this.streamsMutex = new Object();
        this.streams = new HashMap<String, InLongStream>();
        this.pendingStreams = new HashMap<String, InLongStream[]>();
    }
     ...
     */
    public void addStreamListener(StreamListener<InLongStream> listener) {
     ...
     */
    public void removeStreamListener(StreamListener<InLongStream> listener) {
    ...
    public InLongStream[] getStreams()
    {
        InLongStream[] retval = null;
        Iterator<InLongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            retval = new InLongStream[streams.size()];
            int streams_idx = 0;
    ...
    {
        InLongStream stream = new InLongStream(sri, this);
        synchronized (this.streamsMutex) {
            if (!streams.containsKey(streamID)) {
                // New stream
                ...
                if (!pendingStreams.containsKey(streamID)) {
                    pendingStreams.put(streamID, new InLongStream[0]);
                }
                InLongStream[] tmp_streams = Arrays.copyOf(pendingStreams.get(streamID), pendingStreams.get(streamID).length+1);
                tmp_streams[tmp_streams.length - 1] = stream;
                pendingStreams.replace(streamID, tmp_streams);
            }
            for (StreamListener<InLongStream> listener : streamAdded) {
                listener.newStream(stream);
            }
        }
    }

    public InLongStream getCurrentStream(float timeout)
    {
      // Prefer a stream that already has buffered data
      InLongStream retval = null;
      synchronized (this.streamsMutex) {
        for (InLongStream value : streams.values()) {
    ...
    public boolean isStreamEnabled(String streamID)
    {
        synchronized (this.streamsMutex) {
            if (!pendingStreams.containsKey(streamID)) {
                InLongStream stream = streams.get(streamID);
                if (stream != null) {
                    if (!stream.enabled()) {
                        return false;
                    }
    ...
    public InLongStream[] getReadyStreams(int samples)
    {
        InLongStream[] retval = null;
        List<InLongStream> stream_list = new ArrayList<InLongStream>();
        Iterator<InLongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            while (streams_iter.hasNext()) {
                InLongStream _stream = streams_iter.next();
                if (samples == 0) {
                    if (_stream.ready()) {
            ...
            int streams_idx = 0;
            retval = new InLongStream[stream_list.size()];
    ...
    protected boolean _acceptPacket(String streamID, boolean EOS)
    {
        ...
        synchronized (this.streamsMutex) {
            ...
            if (EOS) {
                ...
                InLongStream[] _pS = pendingStreams.get(streamID);
                if (_pS != null) {
                    streams.put(streamID, _pS[0]);
                    InLongStream[] tmp_streams = Arrays.copyOfRange(pendingStreams.get(streamID), 1, pendingStreams.get(streamID).length);
                    pendingStreams.replace(streamID, tmp_streams);
    ...
    protected void _removeStream(String streamID)
    {
        synchronized (this.streamsMutex) {
            ...
            InLongStream value = streams.get(streamID);
            if (value != null) {
                value._close();
                streams.remove(streamID);
            }
