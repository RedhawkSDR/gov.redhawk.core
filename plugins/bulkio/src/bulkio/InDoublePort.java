...
public class InDoublePort extends BULKIO.jni.dataDoublePOA implements InDataPort<BULKIO.dataDoubleOperations,double[]> {
    ...
    protected Map<String, InDoubleStream> streams;
    protected Map<String, InDoubleStream[]> pendingStreams;
    protected List<StreamListener<InDoubleStream>> streamAdded = new LinkedList<StreamListener<InDoubleStream>>();
    ...
    public InDoublePort( String portName,
        ...
        this.streams = new HashMap<String, InDoubleStream>();
        this.pendingStreams = new HashMap<String, InDoubleStream[]>();
    }
     ...
     */
    public void addStreamListener(StreamListener<InDoubleStream> listener) {
     ...
     */
    public void removeStreamListener(StreamListener<InDoubleStream> listener) {
    ...
    public void pushSRI(StreamSRI header) {
        synchronized (impl.sriUpdateLock) {
            if (!impl.currentHs.containsKey(header.streamID)) {
                ...
                int eos_count = 0;
                synchronized (impl.dataBufferLock) {
                    for (DataTransfer<double[]> packet : impl.workQueue) {
    ...
    public InDoubleStream getStream(String streamID)
    {
        InDoubleStream stream = null;
    ...
    public InDoubleStream[] getStreams()
    {
        InDoubleStream[] retval = null;
        Iterator<InDoubleStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            retval = new InDoubleStream[streams.size()];
            int streams_idx = 0;
    ...
    {
        InDoubleStream stream = new InDoubleStream(sri, this);
        synchronized (this.streamsMutex) {
            if (!streams.containsKey(streamID)) {
                // New stream
                ...
                if (!pendingStreams.containsKey(streamID)) {
                    pendingStreams.put(streamID, new InDoubleStream[0]);
                }
                InDoubleStream[] tmp_streams = Arrays.copyOf(pendingStreams.get(streamID), pendingStreams.get(streamID).length+1);
                tmp_streams[tmp_streams.length - 1] = stream;
                pendingStreams.replace(streamID, tmp_streams);
            }
            for (StreamListener<InDoubleStream> listener : streamAdded) {
                listener.newStream(stream);
            }
        }
    }

    public InDoubleStream getCurrentStream(float timeout)
    {
      // Prefer a stream that already has buffered data
      InDoubleStream retval = null;
      synchronized (this.streamsMutex) {
        for (InDoubleStream value : streams.values()) {
    ...
    public Packet peekPacket(float timeout)
    {
        ...
        DataTransfer<double[]> p = impl.peekPacket(timeout_ms);
        if (p != null) {
            return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
        }
        return null;
    ...
    {
        DataTransfer<double[]> p =  this.impl.fetchPacket(streamID);
        if (p == null) {
            return null;
        }
        return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
    ...
    public boolean isStreamEnabled(String streamID)
    {
        synchronized (this.streamsMutex) {
            if (!pendingStreams.containsKey(streamID)) {
                InDoubleStream stream = streams.get(streamID);
                if (stream != null) {
                    if (!stream.enabled()) {
                        return false;
                    }
    ...
    public InDoubleStream[] getReadyStreams(int samples)
    {
        InDoubleStream[] retval = null;
        List<InDoubleStream> stream_list = new ArrayList<InDoubleStream>();
        Iterator<InDoubleStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            while (streams_iter.hasNext()) {
                InDoubleStream _stream = streams_iter.next();
                if (samples == 0) {
                    if (_stream.ready()) {
            ...
            int streams_idx = 0;
            retval = new InDoubleStream[stream_list.size()];
    ...
    protected boolean _acceptPacket(String streamID, boolean EOS)
    {
        ...
        synchronized (this.streamsMutex) {
            ...
            if (EOS) {
                ...
                InDoubleStream[] _pS = pendingStreams.get(streamID);
                if (_pS != null) {
                    streams.put(streamID, _pS[0]);
                    InDoubleStream[] tmp_streams = Arrays.copyOfRange(pendingStreams.get(streamID), 1, pendingStreams.get(streamID).length);
                    pendingStreams.replace(streamID, tmp_streams);
    ...
    protected void _removeStream(String streamID)
    {
        synchronized (this.streamsMutex) {
            ...
            InDoubleStream value = streams.get(streamID);
            if (value != null) {
                value._close();
                streams.remove(streamID);
            }
