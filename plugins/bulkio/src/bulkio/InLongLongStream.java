...
public class InLongLongStream extends InStreamBase {

    protected InLongLongPort _port;
    ...
    public ArrayDeque<InLongLongPort.Packet> _queue;
    
    public InLongLongStream(BULKIO.StreamSRI sri, InLongLongPort port) {
        /*
        Create an InLongLongStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InLongLongPort.Packet>();
    ...
    public LongLongDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public LongLongDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public LongLongDataBlock read(int count, int consume, boolean blocking) {
    ...
    public LongLongDataBlock tryread() {
        /*
    ...
    public LongLongDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public LongLongDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(LongLongDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(LongLongDataBlock block, InLongLongPort.Packet packet)
    {
    ...
    protected LongLongDataBlock _readData(int count, int consume)
    {
        ...
        InLongLongPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        long[] empty_data = new long[0];
        LongLongDataBlock data = new LongLongDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InLongLongPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InLongLongPort.Packet packet = itr.next();
    ...
    protected boolean _queuePacket(InLongLongPort.Packet packet) {
    ...
    protected LongLongDataBlock _readPacket(boolean blocking) {
        InLongLongPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        LongLongDataBlock block = new LongLongDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
    ...
    protected boolean _canBridge(InLongLongPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InLongLongPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InLongLongPort.Packet packet = null;
    ...
    protected InLongLongPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InLongLongPort.Packet packet) {
        if (this._newstream) {
