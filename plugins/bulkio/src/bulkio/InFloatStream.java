...
public class InFloatStream extends InStreamBase {

    protected InFloatPort _port;
    ...
    public ArrayDeque<InFloatPort.Packet> _queue;
    
    public InFloatStream(BULKIO.StreamSRI sri, InFloatPort port) {
        /*
        Create an InFloatStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InFloatPort.Packet>();
    ...
    public FloatDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public FloatDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public FloatDataBlock read(int count, int consume, boolean blocking) {
    ...
    public FloatDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public FloatDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(FloatDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(FloatDataBlock block, InFloatPort.Packet packet)
    {
    ...
    protected FloatDataBlock _readData(int count, int consume)
    {
        ...
        InFloatPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        float[] empty_data = new float[0];
        FloatDataBlock data = new FloatDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InFloatPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InFloatPort.Packet packet = itr.next();
                float[] input_data = packet.dataBuffer;
    ...
    protected boolean _queuePacket(InFloatPort.Packet packet) {
    ...
    protected FloatDataBlock _readPacket(boolean blocking) {
        InFloatPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        FloatDataBlock block = new FloatDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        ...
        if (this._sampleOffset != 0) {
            ...
            float[] data_payload = Arrays.copyOfRange(packet.dataBuffer, this._sampleOffset, packet.dataBuffer.length);
    ...
    protected boolean _canBridge(InFloatPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InFloatPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InFloatPort.Packet packet = null;
    ...
    protected InFloatPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InFloatPort.Packet packet) {
        if (this._newstream) {
    ...
    protected void _consumeData(int count) {
        this._refreshQueue();
        while (count > 0) {
            float[] data = this._queue.peekFirst().dataBuffer;
