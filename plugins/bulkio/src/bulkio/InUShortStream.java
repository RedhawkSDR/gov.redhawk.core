...
public class InUShortStream extends InStreamBase {

    protected InUShortPort _port;
    ...
    public ArrayDeque<InUShortPort.Packet> _queue;
    
    public InUShortStream(BULKIO.StreamSRI sri, InUShortPort port) {
        /*
        Create an InUShortStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InUShortPort.Packet>();
    ...
    public UShortDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public UShortDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public UShortDataBlock read(int count, int consume, boolean blocking) {
    ...
    public UShortDataBlock tryread() {
        /*
    ...
    public UShortDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public UShortDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(UShortDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(UShortDataBlock block, InUShortPort.Packet packet)
    {
    ...
    protected UShortDataBlock _readData(int count, int consume)
    {
        ...
        InUShortPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        short[] empty_data = new short[0];
        UShortDataBlock data = new UShortDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InUShortPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InUShortPort.Packet packet = itr.next();
                short[] input_data = packet.dataBuffer;
    ...
    protected boolean _queuePacket(InUShortPort.Packet packet) {
    ...
    protected UShortDataBlock _readPacket(boolean blocking) {
        InUShortPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        UShortDataBlock block = new UShortDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        ...
        if (this._sampleOffset != 0) {
            ...
            short[] data_payload = Arrays.copyOfRange(packet.dataBuffer, this._sampleOffset, packet.dataBuffer.length);
    ...
    protected boolean _canBridge(InUShortPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InUShortPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InUShortPort.Packet packet = null;
    ...
    protected InUShortPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InUShortPort.Packet packet) {
        if (this._newstream) {
    ...
    protected void _consumeData(int count) {
        this._refreshQueue();
        while (count > 0) {
            short[] data = this._queue.peekFirst().dataBuffer;
