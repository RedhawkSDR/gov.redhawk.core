...
import bulkio.InShortStream;
import bulkio.StreamListener;

/**
 * 
 */
public class InShortPort extends BULKIO.jni.dataShortPOA implements InDataPort<BULKIO.dataShortOperations,short[]> {
    ...
    protected Map<String, InShortStream> streams;
    protected Map<String, InShortStream[]> pendingStreams;
    protected List<StreamListener<InShortStream>> streamAdded = new LinkedList<StreamListener<InShortStream>>();
    ...
    public InShortPort( String portName,
                       ...
                       bulkio.sri.Comparator compareSRI,
                       bulkio.SriListener sriCallback ) {
        impl = new InPortImpl<short[]>(portName, logger, compareSRI, sriCallback, new ShortDataHelper());
        this.streamsMutex = new Object();
        this.streams = new HashMap<String, InShortStream>();
        this.pendingStreams = new HashMap<String, InShortStream[]>();
    }
     ...
     */
    public void addStreamListener(StreamListener<InShortStream> listener) {
     ...
     */
    public void removeStreamListener(StreamListener<InShortStream> listener) {
    ...
    public InShortStream getStream(String streamID)
    {
        InShortStream stream = null;
    ...
    public InShortStream[] getStreams()
    {
        InShortStream[] retval = null;
        Iterator<InShortStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            retval = new InShortStream[streams.size()];
            int streams_idx = 0;
    ...
    {
        InShortStream stream = new InShortStream(sri, this);
        synchronized (this.streamsMutex) {
            if (!streams.containsKey(streamID)) {
                // New stream
                ...
                if (!pendingStreams.containsKey(streamID)) {
                    pendingStreams.put(streamID, new InShortStream[0]);
                }
                InShortStream[] tmp_streams = Arrays.copyOf(pendingStreams.get(streamID), pendingStreams.get(streamID).length+1);
                tmp_streams[tmp_streams.length - 1] = stream;
                pendingStreams.replace(streamID, tmp_streams);
            }
            for (StreamListener<InShortStream> listener : streamAdded) {
                listener.newStream(stream);
            }
        }
    }

    public InShortStream getCurrentStream(float timeout)
    {
      // Prefer a stream that already has buffered data
      InShortStream retval = null;
      synchronized (this.streamsMutex) {
        for (InShortStream value : streams.values()) {
    ...
    public boolean isStreamEnabled(String streamID)
    {
        synchronized (this.streamsMutex) {
            if (!pendingStreams.containsKey(streamID)) {
                InShortStream stream = streams.get(streamID);
                if (stream != null) {
                    if (!stream.enabled()) {
                        return false;
                    }
    ...
    public InShortStream[] getReadyStreams(int samples)
    {
        InShortStream[] retval = null;
        List<InShortStream> stream_list = new ArrayList<InShortStream>();
        Iterator<InShortStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            while (streams_iter.hasNext()) {
                InShortStream _stream = streams_iter.next();
                if (samples == 0) {
                    if (_stream.ready()) {
            ...
            int streams_idx = 0;
            retval = new InShortStream[stream_list.size()];
    ...
    protected boolean _acceptPacket(String streamID, boolean EOS)
    {
        ...
        synchronized (this.streamsMutex) {
            ...
            if (EOS) {
                ...
                InShortStream[] _pS = pendingStreams.get(streamID);
                if (_pS != null) {
                    streams.put(streamID, _pS[0]);
                    InShortStream[] tmp_streams = Arrays.copyOfRange(pendingStreams.get(streamID), 1, pendingStreams.get(streamID).length);
                    pendingStreams.replace(streamID, tmp_streams);
    ...
    protected void _removeStream(String streamID)
    {
        synchronized (this.streamsMutex) {
            ...
            InShortStream value = streams.get(streamID);
            if (value != null) {
                value._close();
                streams.remove(streamID);
            }
