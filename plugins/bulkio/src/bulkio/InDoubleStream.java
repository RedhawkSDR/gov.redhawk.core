...
public class InDoubleStream extends InStreamBase {

    protected InDoublePort _port;
    ...
    public ArrayDeque<InDoublePort.Packet> _queue;
    
    public InDoubleStream(BULKIO.StreamSRI sri, InDoublePort port) {
        /*
        Create an InDoubleStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InDoublePort.Packet>();
    ...
    public DoubleDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public DoubleDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public DoubleDataBlock read(int count, int consume, boolean blocking) {
    ...
    public DoubleDataBlock tryread() {
        /*
    ...
    public DoubleDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public DoubleDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(DoubleDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(DoubleDataBlock block, InDoublePort.Packet packet)
    {
    ...
    protected DoubleDataBlock _readData(int count, int consume)
    {
        ...
        InDoublePort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        double[] empty_data = new double[0];
        DoubleDataBlock data = new DoubleDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InDoublePort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InDoublePort.Packet packet = itr.next();
                double[] input_data = packet.dataBuffer;
    ...
    protected boolean _queuePacket(InDoublePort.Packet packet) {
    ...
    protected DoubleDataBlock _readPacket(boolean blocking) {
        InDoublePort.Packet packet = this._fetchNextPacket(blocking);
        ...
        DoubleDataBlock block = new DoubleDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        ...
        if (this._sampleOffset != 0) {
            ...
            double[] data_payload = Arrays.copyOfRange(packet.dataBuffer, this._sampleOffset, packet.dataBuffer.length);
    ...
    protected boolean _canBridge(InDoublePort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InDoublePort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InDoublePort.Packet packet = null;
    ...
    protected InDoublePort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InDoublePort.Packet packet) {
        if (this._newstream) {
    ...
    protected void _consumeData(int count) {
        this._refreshQueue();
        while (count > 0) {
            double[] data = this._queue.peekFirst().dataBuffer;
