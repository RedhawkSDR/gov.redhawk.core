 ...
 * 
 */
public class OutInt16Port extends BULKIO.UsesPortStatisticsProviderPOA {
     ...
     * @generated
     */
    protected Map<String, dataShortOperations> outConnections = null;
     ...
     */
    protected static int MAX_SAMPLES_PER_PUSH = MAX_PAYLOAD_SIZE/Int16Size.bytes();
    ...
    public OutInt16Port(String portName,
        ...
        active = false;
        outConnections = new HashMap<String, dataShortOperations>();
     ...
     * @generated
     */
    public HashMap<String, dataShortOperations> getPorts() {
        return new HashMap<String, dataShortOperations>();
    }
	...
	if ( logger != null ) {
		...
		// for each connection
		for (Entry<String, dataShortOperations> p : this.outConnections.entrySet()) {
            ...
            String streamID)
    {
        ...
        if (sriStruct.sri.subsize != 0) {
            if (MAX_SAMPLES_PER_PUSH%sriStruct.sri.subsize != 0) {
                MAX_SAMPLES_PER_PUSH = MAX_PAYLOAD_SIZE/Int16Size.bytes();
    ...
    public void connectPort(final org.omg.CORBA.Object connection, final String connectionId) throws CF.PortPackage.InvalidPort, CF.PortPackage.OccupiedPort
    {
        ...
        synchronized (this.updatingPortsLock) {
            final dataShortOperations port;
            try {
                port = BULKIO.jni.dataShortHelper.narrow(connection);
            ...
            this.active = true;
            this.stats.put(connectionId, new linkStatistics( this.name, new Int16Size() ) );
    ...
    public void disconnectPort(String connectionId) {
        ...
        synchronized (this.updatingPortsLock) {
            ...
            dataShortOperations port = this.outConnections.remove(connectionId);
            if (port != null)
            {
    ...
    public UsesConnection[] connections() {
        ...
        synchronized (this.updatingPortsLock) {
            for (Entry<String, dataShortOperations> ent : this.outConnections.entrySet()) {
