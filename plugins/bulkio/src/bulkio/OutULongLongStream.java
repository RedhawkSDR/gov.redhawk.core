...
import bulkio.OutULongLongPort;

import java.util.Arrays;

public class OutULongLongStream extends OutStreamBase {

    protected OutULongLongPort _port;

    public OutULongLongStream(BULKIO.StreamSRI sri, OutULongLongPort port) {
        super(sri);
        this._port = port;
        _bufferSize = 0;
        _bufferOffset = 0;
    ...
    protected void _writeMultiple(long[] data, bulkio.SampleTimestamp[] times, boolean eos) {
        ...
        for (int times_idx = 0; times_idx < times.length; times_idx++) {
            if (times.length == 1) {
                ...
                long[] _data = Arrays.copyOfRange(data, times[times_idx].offset*offset_multiple, end_offset);
                this._port.pushPacket(_data, times[times_idx].time, eos, this._sri.streamID);
            }
        }
    };

    protected void _writeMultiple(CF.complexULongLong[] cx_data, bulkio.SampleTimestamp[] times, boolean eos) {
        if (this.checkSRI()) {
            this._port.pushSRI(this._sri);
        }
        long[] data = new long[cx_data.length*2];
        ...
        for (int times_idx = 0; times_idx < times.length; times_idx++) {
            if (times.length == 1) {
                ...
                long[] _data = Arrays.copyOfRange(data, times[times_idx].offset*2, end_offset);
                this._port.pushPacket(_data, times[times_idx].time, eos, this._sri.streamID);
            }
        }
    };
    ...
    protected void _flush(boolean eos) {
        ...
        if (this._bufferTime != null) {
            ...
            if (this._bufferSize != 0) {
                if ((this._bufferWritePtr != 0) || (eos)) {
                    long[] tmp_buffer = new long[this._bufferWritePtr];
                ...
                if ((this._bufferOffset == 0) && (eos)) {
                    long[] tmp_buffer = new long[0];
                    this._writeMultiple(tmp_buffer, times, eos);
                } else if (this._bufferOffset != 0) {
                    long[] tmp_buffer = new long[this._bufferOffset];
        ...
        this._buffer = new long[this._bufferSize];
        this._bufferTime = null;
    };

    protected void _doBuffer(CF.complexULongLong[] cx_data, BULKIO.PrecisionUTCTime time) {
        long[] data = new long[cx_data.length*2];
        for (int cx_data_idx = 0; cx_data_idx < cx_data.length; cx_data_idx++) {
            data[cx_data_idx*2] = cx_data[cx_data_idx].real;
            data[cx_data_idx*2+1] = cx_data[cx_data_idx].imag;
    ...
    protected void _doBuffer(long[] data, BULKIO.PrecisionUTCTime time) {
        ...
        if (count < data.length) {
            ...
            long [] recursive_data = Arrays.copyOfRange(data, count, data.length);
    ...
    /*
        ...
        */
    public void write(long[] data, BULKIO.PrecisionUTCTime time) {
     ...
     * If there are any pending SRI changes, the new SRI is pushed first.
     */
    public void write(long[] data, bulkio.SampleTimestamp[] times) {
        if ((this._bufferSize == 0) || ((this._bufferWritePtr == 0) && (data.length >= this._bufferSize))) {
        //if (this._bufferSize == 0) {
                this._writeMultiple(data, times, false);
            return;
        ...
        for (int times_idx = 0; times_idx < times.length; times_idx++) {
            if (times.length == 1) {
                ...
                long[] _data = Arrays.copyOfRange(data, times[times_idx].offset, end_offset);
     ...
     * If there are any pending SRI changes, the new SRI is pushed first.
     */
    public void write(CF.complexULongLong[] data, BULKIO.PrecisionUTCTime time) {
        if ((this._bufferSize == 0) || ((this._bufferWritePtr == 0) && (data.length >= this._bufferSize))) {
        //if (this._bufferSize == 0) {
                bulkio.SampleTimestamp[] times = new SampleTimestamp[1];
            times[0] = new bulkio.SampleTimestamp(time, 0, false);
     ...
     * If there are any pending SRI changes, the new SRI is pushed first.
     */
    public void write(CF.complexULongLong[] data, bulkio.SampleTimestamp[] times) {
        if ((this._bufferSize == 0) || ((this._bufferWritePtr == 0) && (data.length >= this._bufferSize))) {
        //if (this._bufferSize == 0) {
                this._writeMultiple(data, times, false);
            return;
        ...
        for (int times_idx = 0; times_idx < times.length; times_idx++) {
            if (times.length == 1) {
                ...
                CF.complexULongLong[] _data = Arrays.copyOfRange(data, times[times_idx].offset, end_offset);
                this._doBuffer(_data, times[times_idx].time);
            }
        }
    };
