...
public class InShortStream extends InStreamBase {

    protected InShortPort _port;
    ...
    public ArrayDeque<InShortPort.Packet> _queue;
    
    public InShortStream(BULKIO.StreamSRI sri, InShortPort port) {
        /*
        Create an InShortStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InShortPort.Packet>();
    ...
    public ShortDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public ShortDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public ShortDataBlock read(int count, int consume, boolean blocking) {
    ...
    public ShortDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public ShortDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(ShortDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(ShortDataBlock block, InShortPort.Packet packet)
    {
    ...
    protected ShortDataBlock _readData(int count, int consume)
    {
        ...
        InShortPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        short[] empty_data = new short[0];
        ShortDataBlock data = new ShortDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InShortPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InShortPort.Packet packet = itr.next();
    ...
    protected boolean _queuePacket(InShortPort.Packet packet) {
    ...
    protected ShortDataBlock _readPacket(boolean blocking) {
        InShortPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        ShortDataBlock block = new ShortDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
    ...
    protected boolean _canBridge(InShortPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InShortPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InShortPort.Packet packet = null;
    ...
    protected InShortPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InShortPort.Packet packet) {
        if (this._newstream) {
