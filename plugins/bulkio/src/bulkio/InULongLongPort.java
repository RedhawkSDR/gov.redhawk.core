...
import bulkio.InULongLongStream;
import bulkio.StreamListener;

/**
 * 
 */
public class InULongLongPort extends BULKIO.jni.dataUlongLongPOA implements InDataPort<BULKIO.dataUlongLongOperations,long[]> {
    ...
    protected Map<String, InULongLongStream> streams;
    protected Map<String, InULongLongStream[]> pendingStreams;
    protected List<StreamListener<InULongLongStream>> streamAdded = new LinkedList<StreamListener<InULongLongStream>>();
    ...
    public InULongLongPort( String portName,
                       ...
                       bulkio.sri.Comparator compareSRI,
                       bulkio.SriListener sriCallback ) {
        impl = new InPortImpl<long[]>(portName, logger, compareSRI, sriCallback, new ULongLongDataHelper());
        this.streamsMutex = new Object();
        this.streams = new HashMap<String, InULongLongStream>();
        this.pendingStreams = new HashMap<String, InULongLongStream[]>();
    }
     ...
     */
    public void addStreamListener(StreamListener<InULongLongStream> listener) {
     ...
     */
    public void removeStreamListener(StreamListener<InULongLongStream> listener) {
    ...
    public void pushSRI(StreamSRI header) {
        synchronized (impl.sriUpdateLock) {
            if (!impl.currentHs.containsKey(header.streamID)) {
                ...
                int eos_count = 0;
                synchronized (impl.dataBufferLock) {
                    for (DataTransfer<long[]> packet : impl.workQueue) {
    ...
    public InULongLongStream getStream(String streamID)
    {
        InULongLongStream stream = null;
    ...
    public InULongLongStream[] getStreams()
    {
        InULongLongStream[] retval = null;
        Iterator<InULongLongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            retval = new InULongLongStream[streams.size()];
            int streams_idx = 0;
    ...
    {
        InULongLongStream stream = new InULongLongStream(sri, this);
        synchronized (this.streamsMutex) {
            if (!streams.containsKey(streamID)) {
                // New stream
                ...
                if (!pendingStreams.containsKey(streamID)) {
                    pendingStreams.put(streamID, new InULongLongStream[0]);
                }
                InULongLongStream[] tmp_streams = Arrays.copyOf(pendingStreams.get(streamID), pendingStreams.get(streamID).length+1);
                tmp_streams[tmp_streams.length - 1] = stream;
                pendingStreams.replace(streamID, tmp_streams);
            }
            for (StreamListener<InULongLongStream> listener : streamAdded) {
                listener.newStream(stream);
            }
        }
    }

    public InULongLongStream getCurrentStream(float timeout)
    {
      // Prefer a stream that already has buffered data
      InULongLongStream retval = null;
      synchronized (this.streamsMutex) {
        for (InULongLongStream value : streams.values()) {
    ...
    public Packet peekPacket(float timeout)
    {
        ...
        DataTransfer<long[]> p = impl.peekPacket(timeout_ms);
        if (p != null) {
            return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
        }
        return null;
    ...
    {
        DataTransfer<long[]> p =  this.impl.fetchPacket(streamID);
        if (p == null) {
            return null;
        }
        return new Packet(p.getData(), p.getTime(), p.getEndOfStream(), p.getStreamID(), p.getSRI(), p.sriChanged(), p.inputQueueFlushed());
    ...
    public boolean isStreamEnabled(String streamID)
    {
        synchronized (this.streamsMutex) {
            if (!pendingStreams.containsKey(streamID)) {
                InULongLongStream stream = streams.get(streamID);
                if (stream != null) {
                    if (!stream.enabled()) {
                        return false;
                    }
    ...
    public InULongLongStream[] getReadyStreams(int samples)
    {
        InULongLongStream[] retval = null;
        List<InULongLongStream> stream_list = new ArrayList<InULongLongStream>();
        Iterator<InULongLongStream> streams_iter = streams.values().iterator();
        synchronized (this.streamsMutex) {
            while (streams_iter.hasNext()) {
                InULongLongStream _stream = streams_iter.next();
                if (samples == 0) {
                    if (_stream.ready()) {
            ...
            int streams_idx = 0;
            retval = new InULongLongStream[stream_list.size()];
    ...
    protected boolean _acceptPacket(String streamID, boolean EOS)
    {
        ...
        synchronized (this.streamsMutex) {
            ...
            if (EOS) {
                ...
                InULongLongStream[] _pS = pendingStreams.get(streamID);
                if (_pS != null) {
                    streams.put(streamID, _pS[0]);
                    InULongLongStream[] tmp_streams = Arrays.copyOfRange(pendingStreams.get(streamID), 1, pendingStreams.get(streamID).length);
                    pendingStreams.replace(streamID, tmp_streams);
    ...
    protected void _removeStream(String streamID)
    {
        synchronized (this.streamsMutex) {
            ...
            InULongLongStream value = streams.get(streamID);
            if (value != null) {
                value._close();
                streams.remove(streamID);
            }
