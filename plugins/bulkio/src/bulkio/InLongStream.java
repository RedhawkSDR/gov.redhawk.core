...
public class InLongStream extends InStreamBase {

    protected InLongPort _port;
    ...
    public ArrayDeque<InLongPort.Packet> _queue;
    
    public InLongStream(BULKIO.StreamSRI sri, InLongPort port) {
        /*
        Create an InLongStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InLongPort.Packet>();
    ...
    public LongDataBlock read(int count) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public LongDataBlock read(int count, int consume) {
        /*
        Blocking read of the next packet for this stream.
    ...
    public LongDataBlock read(int count, int consume, boolean blocking) {
    ...
    public LongDataBlock tryread(int count) {
        /*
        Non-blocking version of read().
    ...
    public LongDataBlock tryread(int count, int consume) {
        /*
        Non-blocking version of read().
    ...
    protected void _addTimestamp(LongDataBlock data, int inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(LongDataBlock block, InLongPort.Packet packet)
    {
    ...
    protected LongDataBlock _readData(int count, int consume)
    {
        ...
        InLongPort.Packet front = this._queue.peekFirst();

        // Allocate empty data block and propagate the SRI change and input queue
        // flush flags
        int[] empty_data = new int[0];
        LongDataBlock data = new LongDataBlock(front.SRI, empty_data, 0, false);
        ...
        if (last_offset <= front.dataBuffer.length) {
            ...
            Iterator<InLongPort.Packet> itr = _queue.iterator();
            while (count > 0) {
                while (!itr.hasNext()) {
                    try {
                        Thread.sleep(1);
                ...
                InLongPort.Packet packet = itr.next();
    ...
    protected boolean _queuePacket(InLongPort.Packet packet) {
    ...
    protected LongDataBlock _readPacket(boolean blocking) {
        InLongPort.Packet packet = this._fetchNextPacket(blocking);
        ...
        LongDataBlock block = new LongDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
    ...
    protected boolean _canBridge(InLongPort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InLongPort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InLongPort.Packet packet = null;
    ...
    protected InLongPort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InLongPort.Packet packet) {
        if (this._newstream) {
