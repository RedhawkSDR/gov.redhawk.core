...
public final class StatusUtil {
	 ...
	 * @return the list
	 */
	protected static List<IStatus> flatten(final IStatus aStatus) {
		...
		if (aStatus.isMultiStatus()) {
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (currentChild.isMultiStatus()) {
					final Iterator<IStatus> childStatiiEnum = StatusUtil.flatten(currentChild).iterator();
	...
	/**
	 * This method must not be called outside the workbench. Utility method for
	 * creating status.
	 * 
	...
	protected static IStatus newStatus(final IStatus[] stati, final String message, final Throwable exception) {
		...
		return new MultiStatus(RedhawkUiActivator.getPluginId(), IStatus.OK, stati, message, exception);
	}
	...
	public static Throwable getCause(final Throwable exception) {
		...
		if (exception != null) {
			if (exception instanceof CoreException) {
				...
				// cause. Otherwise, use the general-purpose mechanism. Remove 
				// this branch if CoreException ever implements a correct 
				// getCause() method.
				final CoreException ce = (CoreException) exception;
	...
	public static IStatus newStatus(final int severity, final String message, final Throwable exception) {
		...
		return new Status(severity, RedhawkUiActivator.getPluginId(), severity, statusMessage, StatusUtil.getCause(exception));
	}
	 ...
	 * @return the i status
	 */
	public static IStatus newStatus(final List<IStatus> children, final String message, final Throwable exception) {

		final List<IStatus> flatStatusCollection = new ArrayList<IStatus>();
		final Iterator<IStatus> iter = children.iterator();
		while (iter.hasNext()) {
			final IStatus currentStatus = iter.next();
			final Iterator<IStatus> childrenIter = StatusUtil.flatten(currentStatus).iterator();
	...
	public static void handleStatus(final Throwable e, final int hint) {
		StatusManager.getManager().handle(StatusUtil.newStatus(RedhawkUiActivator.getPluginId(), e), hint);
	}
	...
	public static void handleStatus(final String message, final Throwable e, final int hint) {
		StatusManager.getManager().handle(StatusUtil.newStatus(RedhawkUiActivator.getPluginId(), message, e), hint);
	}
