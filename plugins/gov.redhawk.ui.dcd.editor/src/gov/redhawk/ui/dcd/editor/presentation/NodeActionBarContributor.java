...
/**
 * This is the action bar contributor for the Dcd model editor.
 */
public class NodeActionBarContributor extends EditingDomainActionBarContributor implements ISelectionChangedListener {
	/**
	...
	private final IAction showPropertiesViewAction = new Action("Show &Properties View") {
		...
		public void run() {
			try {
				...
				StatusManager.getManager().handle(new Status(IStatus.ERROR, NodeExplorerPlugin.getPluginID(), "Failed to open Property View.", exception),
	...
	private final IAction refreshViewerAction = new Action("&Refresh") {
		...
		public boolean isEnabled() {
			return NodeActionBarContributor.this.activeEditorPart instanceof IViewerProvider;
		}

		@Override
		public void run() {
			if (NodeActionBarContributor.this.activeEditorPart instanceof IViewerProvider) {
				final Viewer viewer = ((IViewerProvider) NodeActionBarContributor.this.activeEditorPart).getViewer();
				if (viewer != null) {
					viewer.refresh();
				}
			}
	 ...
	 */
	public NodeActionBarContributor() {
	...
	public void contributeToToolBar(final IToolBarManager toolBarManager) {
		toolBarManager.add(new Separator("dcd-settings"));
		toolBarManager.add(new Separator("dcd-additions"));
	}
	...
	public void contributeToMenu(final IMenuManager menuManager) {
		...
		final IMenuManager submenuManager = new MenuManager("&Dcd Editor", "mil.jpeojtrs.sca.dcdMenuID");
	...
	public void selectionChanged(final SelectionChangedEvent event) {
		...
		if (selection instanceof IStructuredSelection && ((IStructuredSelection) selection).size() == 1) {
			...
			final IEditingDomainProvider provider = (IEditingDomainProvider) this.activeEditorPart.getAdapter(IEditingDomainProvider.class);
