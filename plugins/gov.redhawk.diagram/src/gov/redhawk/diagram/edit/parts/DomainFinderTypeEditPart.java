...
import gov.redhawk.diagram.ui.tools.ComboDirectEditManager;
 ...
 * 
 */
public class DomainFinderTypeEditPart extends CompartmentEditPart implements ITextAwareEditPart {

	/**
	* 
	*/
	public static final int VISUAL_ID = 15015;
	...
	* 
	*/
	public DomainFinderTypeEditPart(final View view, final PartitioningElementTypes elementTypes, final PartitioningVisualIDRegistry visualIdRegistry) {
		super(view);
	...
	public IParser getParser() {
		if (this.parser == null) {
			this.parser = PartitioningParserProviderHelper.getParser(PartitioningElementTypes.DomainFinder, getParserElement(),
			        this.visualIdRegistry.getType(DomainFinderTypeEditPart.VISUAL_ID));
		}
		return this.parser;
	}
	...
	*/
	protected DirectEditManager getManager() {
		if (this.manager == null) {
			final List<DomainFinderType> values = DomainFinderType.VALUES;
			final List<String> strValues = new ArrayList<String>(values.size());
			for (final DomainFinderType t : values) {
				strValues.add(t.getLiteral());
			}
			setManager(new ComboDirectEditManager(this, PartitioningEditPartFactory.getTextCellEditorLocator(this), strValues.toArray(new String[strValues
			        .size()])));
		}
		return this.manager;
	}
	...
	*/
	private void performDirectEdit(final char initialCharacter) {
		if (getManager() instanceof ComboDirectEditManager) {
			((ComboDirectEditManager) getManager()).show(initialCharacter);
		} else {
			performDirectEdit();
		}
	}
